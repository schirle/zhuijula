...
// ── AI 助手（OpenAI 兼容接口，默认对接「硅基流动 SiliconFlow」）──
// 环境变量（Cloudflare Pages 后台 / wrangler.toml / .dev.vars 设置）：
//   AI_API_KEY  必填，硅基流动控制台获取的 sk-xxx
//   AI_MODEL    选填，默认 deepseek-ai/DeepSeek-V3；可换 Qwen/Qwen2.5-7B-Instruct 等
//   AI_BASE_URL 选填，默认 https://api.siliconflow.cn/v1（可换成任意 OpenAI 兼容端点）
// 仅未配置 AI_API_KEY 时 isAIConfigured 返回 false、callAI 返回 null，调用方据此降级，绝不抛错
const DEFAULT_AI_BASE = 'https://api.siliconflow.cn/v1';
const DEFAULT_AI_MODEL = 'deepseek-ai/DeepSeek-V3';
export const isAIConfigured = (env) => !!(env && env.AI_API_KEY);

// messages: [{role, content}]；jsonMode: 要求模型只返回 JSON。失败/未配置均返回 null
export async function callAI(messages, env, { temperature = 0.3, jsonMode = true, timeout = 20000 } = {}) {
  if (!isAIConfigured(env)) return null;
  const base = String(env.AI_BASE_URL || DEFAULT_AI_BASE).replace(/\/+$/, '');
  const url = base.endsWith('/chat/completions') ? base : base + '/chat/completions';
  const body = { model: env.AI_MODEL || DEFAULT_AI_MODEL, messages, temperature };
  ...
  try {
    ...
    if (jsonMode) { try { return JSON.parse(content); } catch (_) { return null; } }
    return content;
  } catch (_) { return null; }
