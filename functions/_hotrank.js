...
// 自动清理：删除最后搜索时间距今超过 maxAge 的词（无时间戳的旧数据一并清除）
// 返回被删除的词数量；KV 未绑定或无需清理时返回 0。Cloudflare Pages 无 cron，
// 故在各读写入口（recordHot / 查询 / 后台）主动调用，达到「超过 N 天自动删除」效果。
export const HOT_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;
export async function pruneHot(env, maxAgeMs = HOT_MAX_AGE_MS) {
  const kv = getKV(env);
  if (!kv) return 0;
  const map = await readHotMap(env);
  const ts = await readHotTs(env);
  ...
  for (const w of Object.keys(map)) {
    const t = Number(ts[w]) || 0;
    if (!t || t < cutoff) { delete map[w]; delete ts[w]; removed++; }
  }
  if (removed) {
    try { await writeHotMap(env, map); await writeHotTs(env, ts); } catch { return 0; }
  }
  return removed;
}
