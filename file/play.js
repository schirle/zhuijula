(() => {
    ...
    (() => {
        ...
        const renderDetail = info => {
            ...
            loadAds();
            parseEpisodes(info);
            if (vodName && vodName !== '未知影片') { buildRelated(vodName); loadAiRelate(vodName); }
        };
        ...
        // ── AI 相关搜索（播放页）：请求 /api/ai-relate 生成可点击搜索词，点击跳首页检索 ──
        const loadAiRelate = async name => {
            const block = $('#ai-relate-block');
            const el = $('#ai-relate');
            if (!block || !el) return;
            try {
                const r = await fetch(`/api/ai-relate?title=${encodeURIComponent(name)}`);
                const data = r.ok ? await r.json() : { code: 0 };
                if (!data || data.code !== 1 || !Array.isArray(data.items) || !data.items.length) {
                ...
                data.items.forEach(it => {
                    ...
                    b.type = 'button';
                    b.className = 'ai-chip';
                    b.textContent = it.label || it.query;
                    b.title = '搜索：' + it.query;
                    b.addEventListener('click', () => { location.href = '/?key=' + encodeURIComponent(it.query); });
                    frag.appendChild(b);
                });
                el.appendChild(frag);
                block.style.display = '';
